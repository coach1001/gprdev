angular.module('appCg').controller('DoAssessmentAssessCtrl',function(template,gprRestApi,$stateParams,assessmentInfo){
  var vm=this;
  vm.template = angular.copy(template[0]);
  vm.assessmentInfo = angular.extend(assessmentInfo[0]);
  ...
  vm.saveAnswers = function(){
    gprRestApi.updateCreateRow('application_assessors',{ id : vm.assessmentInfo.id , complete : vm.assessmentInfo.complete},'Update');
    angular.forEach(vm.template.categories, function(cat, catkey) {
      angular.forEach(cat.questions,function(q,qkey){
        if(q.compliance_answers.length){
          if(q.compliance_answers[0].id){
            ...
            q.compliance_answers[0].application_assessor = $stateParams.appassId;
